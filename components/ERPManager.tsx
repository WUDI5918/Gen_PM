...
// 1. Redesigned Toolbox Item (More compact and professional)
const ToolboxItem = ({ type, label, icon: Icon, onClick, colorClass = "text-slate-600 group-hover:text-indigo-600" }: any) => (
    <button 
        onClick={() => onClick(type)}
        className="group flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-lg shadow-sm hover:shadow-md hover:border-indigo-300 hover:bg-indigo-50/50 transition-all duration-200 min-w-[110px]"
    >
        <div className={`p-1.5 rounded-md bg-slate-50 group-hover:bg-white transition-colors ${colorClass}`}>
            <Icon size={16} />
        </div>
        <span className="text-xs font-bold text-slate-700 group-hover:text-indigo-800">{label}</span>
        <Plus size={12} className="ml-auto opacity-0 group-hover:opacity-100 text-indigo-400 transition-opacity" />
    </button>
);
      ...
      <main className="flex-1 overflow-hidden relative flex flex-col">
        ...
        {activeTab === 'builder' && (
            <div className="flex flex-col h-full animate-in fade-in duration-300">
                {/* 1. Improved Top Toolbar (Grouped Categories) */}
                <div className="bg-white border-b border-gray-200 px-6 py-4 shrink-0 z-20 shadow-sm">
                    <div className="max-w-7xl mx-auto flex flex-wrap items-center gap-6">
                        
                        {/* Group: Inputs */}
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] font-extrabold text-gray-300 uppercase tracking-wider mr-1">Inputs</span>
                            <ToolboxItem type="text" label="Text" icon={Type} onClick={addField} colorClass="text-blue-500 bg-blue-50 group-hover:bg-white" />
                            <ToolboxItem type="number" label="Number" icon={Hash} onClick={addField} colorClass="text-emerald-500 bg-emerald-50 group-hover:bg-white" />
                            <ToolboxItem type="date" label="Date" icon={Calendar} onClick={addField} colorClass="text-orange-500 bg-orange-50 group-hover:bg-white" />
                        </div>
                        ...
                        {/* Group: Selections */}
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] font-extrabold text-gray-300 uppercase tracking-wider mr-1">Choices</span>
                            <ToolboxItem type="select" label="Select" icon={List} onClick={addField} colorClass="text-purple-500 bg-purple-50 group-hover:bg-white" />
                            <ToolboxItem type="radio" label="Radio" icon={CircleIcon} onClick={addField} colorClass="text-pink-500 bg-pink-50 group-hover:bg-white" />
                            <ToolboxItem type="checkbox" label="Checkbox" icon={CheckSquare} onClick={addField} colorClass="text-teal-500 bg-teal-50 group-hover:bg-white" />
                        </div>
                        ...
                        {/* Group: Layout */}
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] font-extrabold text-gray-300 uppercase tracking-wider mr-1">Layout</span>
                            <ToolboxItem type="divider" label="Divider" icon={Minus} onClick={addField} colorClass="text-gray-500 bg-gray-100 group-hover:bg-white" />
                            <ToolboxItem type="notice" label="Notice" icon={Bell} onClick={addField} colorClass="text-amber-500 bg-amber-50 group-hover:bg-white" />
                            <ToolboxItem type="spacer" label="Spacer" icon={MoveVertical} onClick={addField} colorClass="text-gray-400 bg-gray-50 group-hover:bg-white" />
                        </div>
                    </div>
                </div>

                {/* 2. Main Layout: Left Canvas | Right Preview */}
                <div className="flex-1 flex overflow-hidden">
                    
                    {/* Left: Canvas (Builder List) */}
                    <div className="flex-1 bg-slate-50 border-r border-gray-200 overflow-y-auto p-6 md:p-8 custom-scrollbar" id="canvas-container" onClick={() => setActiveFieldId(null)}>
                        <div className="max-w-2xl mx-auto">
                            <h3 className="text-xs font-extrabold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                                <Settings2 size={14}/> Form Configuration
                            </h3>
                            {schema.length === 0 ? (
                                <div className="h-64 border-2 border-dashed border-slate-300 rounded-2xl flex flex-col items-center justify-center text-slate-400 bg-slate-100/50">
                                    <LayoutTemplate size={48} className="mb-4 opacity-50" />
                                    <p className="font-bold">Form is Empty</p>
                    ...
                    {/* Right: Live Preview */}
                    <div className="w-[480px] bg-white border-l border-gray-200 overflow-y-auto p-8 shadow-[0_0_40px_-10px_rgba(0,0,0,0.1)] z-10 hidden lg:block custom-scrollbar">
